...
class Env:
    ...
    def state_reset(self, sd=random.random()):
        random.seed(sd)
        minXY = 3 * self.pixel_meter
        maxXY = self.map_dim * self.pixel_meter * 3 / 8
        minVxy = 0.05
        maxVxy = 0.2
        XY_theta = random.random() * 2 * pi
        XY = ((maxXY - minXY) * random.random() + minXY) * array([cos(XY_theta), sin(XY_theta)])
        while self.map.false_loc(XY):
            XY_theta = random.random() * 2 * pi
            XY = ((maxXY - minXY) * random.random() + minXY) * array([cos(XY_theta), sin(XY_theta)])
        ...
        t_d2 = sqrt(2 * (0.2 - self.state[2]) / g[2])
        XY_luo = self.state[0:2] + self.state[3:5] * t_d2

        return self.observe_state(), self.map.get_local_index(XY_luo)
    ...
    def step(self, action):
        ...
        successed, stop_bool, over_speed, over_map, false_loc = False, False, False, False, False
        if validated:
            ...
            if not overtime:
                ...
                false_loc = self.map.false_loc(XY_luo)
                stop_bool = self.energy() < MIN_ENERGY or linalg.norm(self.state[3:5]) < 1e-2 or abs(
                    g[2] * t_fly) < 1e-2
                over_map = linalg.norm(XY_luo) > (self.map_dim * self.pixel_meter / 2)
                over_speed = linalg.norm(self.state[3:5]) > MAX_VXY or self.state[2] > 50
                successed = linalg.norm(self.state[0:2]) < self.r_obj
        reward_value = self.reward(successed, stop_bool, pre_state, pre_t, over_speed, over_map, validated, overtime, false_loc)
        done = successed or stop_bool or over_map or over_speed or overtime or (not validated) or false_loc

        t_d2 = sqrt(2 * (0.2 - self.state[2]) / g[2])
        XY_luo = self.state[0:2] + self.state[3:5] * t_d2
        return self.observe_state(), self.map.get_local_index(XY_luo), reward_value, done
    ...
    def reward(self, successed, stop_bool, pre_state, pre_t, over_speed, over_map, validated, overtime, false_loc):
        def _cos_vec(a, b):
